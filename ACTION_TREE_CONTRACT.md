...
| `ACTION.TENANT.SAVE` | Tenant edit | UPDATE | `app/tenants/page.tsx` | `saveBusinessData` -> `tenants` and related records | UI_PENDING_GUARD | NO_CONFIRM_REQUIRED | ALERT / LOCAL_STATE + CACHE_INVALIDATION | tenant permission | HIGH | tenant page | compatibility edit service | MIGRATION_PENDING |
| `ACTION.TENANT.CREATE` | Atomic tenant create with optional separated receipt | CREATE, FINANCIAL | `app/tenants/page.tsx` | `/api/tenants/create` -> `create_tenant_atomic` | CLIENT_REQUEST_ID + UI_PENDING_GUARD | NO_CONFIRM_REQUIRED | ALERT / LOCAL_STATE + one cache invalidation | tenant/room/contract/payment/deposit permissions | HIGH | tenant-create API/RPC | tenant create action root | MIGRATION_PENDING |
...
| `ACTION.TENANT.DELETE` | Disabled in Closed Beta; tenant history is preserved through move-out/archive | DESTRUCTIVE, DISABLED | Tenant Detail / server boundary | `business-data` rejects with `TENANT_PERMANENT_DELETE_DISABLED` | NO_UI_ENTRY; SERVER_REJECT | N/A | ALERT / NO_MUTATION | tenant lifecycle contract | HIGH | tenant page/API | tenant lifecycle service | CLOSED / PRESERVE_HISTORY |
...
| `ACTION.RENT_PAYMENT.VOID` | Void payment and exact new linked deposit | UPDATE, FINANCIAL, DESTRUCTIVE | Rent Payments | `/api/rent-payments/lifecycle` -> atomic RPC | UI_PENDING_GUARD; final-state replay | CONSEQUENCE_CONFIRM | ALERT / LOCAL_STATE + CACHE_INVALIDATION | payment archive + linked deposit archive permission | HIGH | lifecycle API/RPC | linked receipt lifecycle | MIGRATION_PENDING |
| `ACTION.RENT_PAYMENT.DELETE` | Delete payment and exact new linked deposit | DESTRUCTIVE, FINANCIAL | Rent Payments | `/api/rent-payments/lifecycle` -> atomic RPC; external file cleanup after DB commit | UI_PENDING_GUARD; second call not-found | CONSEQUENCE_CONFIRM | ALERT / LOCAL_STATE + CACHE_INVALIDATION | payment delete + linked deposit delete permission | HIGH | lifecycle API/RPC | linked receipt lifecycle | MIGRATION_PENDING |
...
| `ACTION.TENANT.DELETE` / property/room delete | tenant hard delete is disabled; property/room retain their separate relation guards and recovery hooks | NON_ATOMIC | HIGH / NO_CHANGE_REQUIRED | Tenant rows are tenancy occupants with required property/room links; move-out/archive preserve history and the server rejects legacy tenant hard-delete requests. |
