...
### Current coverage and open-debt period contract

`latestValidRentPeriodPayment` and `getLatestRentPeriodState` select the
current coverage period by coverage end, then coverage start; record entry time
is only a stable tie-breaker. `getOpenRentDebtPeriodStates` separately returns
every valid, expired, payment-specific period whose collection follow-up stays
open. A newer coverage period never erases an older debt by itself; only that
payment's collection, void or waiver can close it. Reminder Engine consumes
open debt periods for debt reminders and the latest period for future rent
...
`lib/debt-case.ts` is the only conversion from payment-specific Rent Domain facts into actionable debt UI data. Reminder Engine, tenant-list debt presentation and every debt surface consume `DebtCase`; they must not recompute debt from coverage dates, waiver IDs or room occupancy. `DebtDisplayModel` and `DebtRow` provide the shared compact two-line presentation. Debt navigation carries `tenantId`, `paymentId` and `focus=debt` so the tenant page opens only the focused Debt Action Panel.
...
## Check-in receipt finance compatibility boundary (2026-08-24)
