...
- 退租表示当前租住关系结束。
- 退租不自动结清、放弃追缴或删除历史欠费；未归档且仍未处理的历史欠费可继续进入日常欠费提醒。
- 归档仅是管理/显示状态：归档租客的日常欠费提醒静默，但归档不改变合同、收款、押金、waiver、历史欠费事实或剩余金额；恢复归档后，未处理欠费可重新进入提醒候选。
...
- 新 Google Drive 附件的 JPEG、PNG、HEIC、HEIF、PDF 统一最大为 4MB，保证应用受控查看和下载不依赖未验证的大响应流行为；历史 Supabase 附件继续遵循其原有 Storage 限制。
- 一个合同、一笔收款或一笔支出可以有零个、一个或多个独立附件。新增附件只追加，不覆盖已有附件；每个附件可独立查看、下载和删除。
...
- 租金覆盖到期的日常提醒遵循当前提醒策略：归档租客静默；退租租客如仍有未处理欠费可继续产生欠费提醒。无论提醒是否展示，退租、归档、恢复归档都不得改变历史欠费事实。
...
- JPEG、JPG、PNG、HEIC、HEIF 和 PDF 在不超过 4MB 时保留原始文件内容、格式、文件名和大小；系统不自动压缩、缩放或转换图片。
- 超过 4MB 的图片会在前端明确提示后生成清晰 JPEG 副本：最长边最多 2500px（原图较小时不放大），质量为 0.90 或 0.85；如果副本仍超限则拒绝。PDF 不压缩，前端和服务端继续拒绝超过 4MB 的 PDF。
- 附件入口支持一次选择多个文件，按选择顺序逐个上传；每个文件独立建立附件记录。
