...
export async function loadAttachmentSummary(workspaceOwnerId: string): Promise<AttachmentSummary> {
  ...
  const byTable = Object.fromEntries((["property_files", "contract_files", "rent_payment_files", "expense_files"] as const).map((table) => {
    const rows = managedItems.filter((item) => item.sourceTable === table);
    return [table, { count: rows.length, bytes: rows.reduce((sum, row) => sum + bytes(row.fileSize), 0) }];
  })) as AttachmentSummary["supabase"]["byTable"];
  const byType = {
    image: { count: managedItems.filter((item) => isImage(item.fileType)).length, bytes: managedItems.filter((item) => isImage(item.fileType)).reduce((sum, item) => sum + bytes(item.fileSize), 0) },
    pdf: { count: managedItems.filter((item) => isPdf(item.fileType)).length, bytes: managedItems.filter((item) => isPdf(item.fileType)).reduce((sum, item) => sum + bytes(item.fileSize), 0) },
    other: { count: managedItems.filter((item) => !isImage(item.fileType) && !isPdf(item.fileType)).length, bytes: managedItems.filter((item) => !isImage(item.fileType) && !isPdf(item.fileType)).reduce((sum, item) => sum + bytes(item.fileSize), 0) }
  };
  ...
  return {
    ...
    supabase: { totalCount: managedItems.length, totalBytes: managedItems.reduce((sum, item) => sum + bytes(item.fileSize), 0), byTable, byType, inRent: { count: 0, bytes: 0 }, movedOut: { count: candidateData.attachmentCount, bytes: candidateData.bytes } },
