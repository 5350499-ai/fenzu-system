...
export type AttachmentInventoryItem = { id: string; sourceTable: AttachmentTable; provider: AttachmentProvider; fileName: string; fileType: string; fileSize: number; uploadedAt: string | null; businessDate: string | null; tenantName: string | null; propertyName: string | null; roomName: string | null; categoryLabel: string; tenantId: string | null };
...
async function loadTenants(ownerId: string) {
  ...
  if (!/move_in_date|actual_move_out_date|column|schema cache/i.test(result.error.message)) throw new Error(`读取租客失败：${result.error.message}`);
...
async function loadContext(ownerId: string) {
  ...
  const [tenants, properties, rooms, contracts, payments, expenses] = await Promise.all([
    ...
    admin.from("rooms").select("id,name,room_number,property_id").eq("user_id", ownerId),
    admin.from("contracts").select("id,tenant_id,property_id,room_id,start_date").eq("user_id", ownerId),
    admin.from("rent_payments").select("id,tenant_id,property_id,room_id,payment_date,rent_month").eq("user_id", ownerId),
    admin.from("expenses").select("id,property_id,room_id,payment_date,expense_month").eq("user_id", ownerId)
  ]);
  ...
  return {
    tenants,
    properties: new Map((properties.data || []).map((row) => [row.id, row as PropertyRow])),
    rooms: new Map((rooms.data || []).map((row) => [row.id, row as RoomRow])),
    contracts: new Map((contracts.data || []).map((row) => [row.id, row as ContractRow])),
    payments: new Map((payments.data || []).map((row) => [row.id, row as PaymentRow])),
    expenses: new Map((expenses.data || []).map((row) => [row.id, row as ExpenseRow]))
  };
}

async function loadAllRows(ownerId: string) {
  const admin = getSupabaseAdmin();
  const rows: Array<{ table: AttachmentTable; row: AttachmentRow }> = [];
  for (const table of ["contract_files", "rent_payment_files", "expense_files"] as const) {
    const result = await admin.from(table).select("*").eq("user_id", ownerId).order("uploaded_at", { ascending: true });
    if (result.error) throw new Error(`读取${table}失败：${result.error.message}`);
...
function relatedRecords(table: AttachmentTable, row: AttachmentRow, context: Awaited<ReturnType<typeof loadContext>>) {
  ...
  const tenant = tenantId ? context.tenants.find((item) => item.id === tenantId) : null;
  ...
  const businessDate = table === "contract_files" ? contract?.start_date : table === "rent_payment_files" ? payment?.payment_date || payment?.rent_month : expense?.payment_date || expense?.expense_month;
...
function inventoryItem(table: AttachmentTable, row: AttachmentRow, context: Awaited<ReturnType<typeof loadContext>>): AttachmentInventoryItem {
  const related = relatedRecords(table, row, context);
  const provider: AttachmentProvider = row.storage_provider === "supabase" || row.storage_provider === "google_drive" ? row.storage_provider : "unknown";
  const categoryLabel = table === "contract_files" ? "合同" : table === "rent_payment_files" ? "收款" : "房屋支出";
  return { id: row.id, sourceTable: table, provider, fileName: row.file_name, fileType: row.file_type || "文件", fileSize: bytes(row.file_size), uploadedAt: formatDate(row.uploaded_at), businessDate: related.businessDate, tenantName: related.tenant?.name || null, propertyName: related.property?.name || related.property?.address || null, roomName: related.room?.room_number || related.room?.name || null, categoryLabel, tenantId: related.tenantId };
}

export async function loadAttachmentInventory(ownerId: string) {
  const context = await loadContext(ownerId);
  const rows = await loadAllRows(ownerId);
  return rows.map(({ table, row }) => inventoryItem(table, row, context));
}

export async function loadAttachmentCleanupCandidates(ownerId: string): Promise<AttachmentCleanupCandidate[]> {
  const context = await loadContext(ownerId);
  const rows = await loadAllRows(ownerId);
  const byTenant = new Map<string, AttachmentRow[]>();
  for (const { table, row } of rows) {
    const related = relatedRecords(table, row, context);
    if (related.tenantId) byTenant.set(related.tenantId, [...(byTenant.get(related.tenantId) || []), row]);
  }
  return context.tenants.filter(isMovedOut).flatMap((tenant) => {
    const tenantRows = byTenant.get(tenant.id) || [];
    if (!tenantRows.length) return [];
    const property = tenant.property_id ? context.properties.get(tenant.property_id) : null;
    const room = tenant.room_id ? context.rooms.get(tenant.room_id) : null;
    return [{ tenantId: tenant.id, tenantName: tenant.name || "未命名租客", propertyName: property?.name || property?.address || "未分类房源", roomName: room?.room_number || room?.name || "未分类房间", status: tenant.status || "", actualMoveOutDate: tenant.actual_move_out_date || null, attachmentCount: tenantRows.length, bytes: tenantRows.reduce((total, row) => total + bytes(row.file_size), 0), googleDriveCount: tenantRows.filter((row) => row.storage_provider === "google_drive").length }];
  });
}
...
export async function cleanupAttachmentIds(ownerId: string, attachmentIds: string[]) {
  const wanted = new Set(attachmentIds.filter(Boolean));
  const rows = (await loadAllRows(ownerId)).filter(({ row }) => wanted.has(row.id));
  return deleteAttachmentRows(ownerId, rows);
}

export async function cleanupTenantAttachments(ownerId: string, tenantIds: string[]) {
  const selected = new Set(tenantIds.filter(Boolean));
  const context = await loadContext(ownerId);
  const rows = (await loadAllRows(ownerId)).filter(({ table, row }) => selected.has(relatedRecords(table, row, context).tenantId || ""));
  return deleteAttachmentRows(ownerId, rows);
}
