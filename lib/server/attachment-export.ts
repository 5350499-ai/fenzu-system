...
const ATTACHMENT_TABLES = ["property_files", "contract_files", "rent_payment_files", "expense_files"] as const;
type AttachmentTable = (typeof ATTACHMENT_TABLES)[number];

type AttachmentRow = {
  id: string; user_id: string; property_id?: string | null; contract_id?: string | null; tenant_id?: string | null;
  rent_payment_id?: string | null; expense_id?: string | null; storage_provider?: string | null;
  storage_bucket?: string | null; storage_path?: string | null; provider_file_id?: string | null;
  file_name: string; file_type: string; file_size: number | null; uploaded_at: string | null;
};
...
export type AttachmentExportManifestEntry = {
  attachmentId: string; sourceTable: AttachmentTable;
  parentType: "property" | "tenant" | "contract" | "rentPayment" | "expense" | "unknown";
  parentId: string | null; tenantId: string | null; contractId: string | null; rentPaymentId: string | null; expenseId: string | null;
  storageProvider: "supabase" | "google_drive" | "unknown"; bucket: string | null; storagePath: string | null; providerFileId: string | null;
  fileName: string; mimeType: string; fileSize: number; uploadedAt: string | null; checksum: string | null;
  archiveRelativePath: string | null; zipPath: string | null; exportStatus: "success" | "missing" | "failed";
  failureReason?: string; status: "exported" | "missing" | "failed"; error?: string;
};
...
function parentFor(table: AttachmentTable, row: AttachmentRow) {
  if (table === "property_files") return { parentType: "property" as const, parentId: row.property_id || null };
...
async function loadRows(admin: ReturnType<typeof getSupabaseAdmin>, ownerId: string) {
  ...
  for (const table of ATTACHMENT_TABLES) {
    ...
    if (error) {
      const missingPropertyTable = table === "property_files" && /relation|table|schema cache|does not exist/i.test(error.message || "");
      if (missingPropertyTable) continue;
      throw new Error(`${table} 附件索引读取失败：${error.message}`);
    }
...
function readablePath(table: AttachmentTable, row: AttachmentRow, context: Awaited<ReturnType<typeof loadContext>>, date: string | null | undefined, used: Set<string>) {
  ...
  const propertyId = row.property_id || tenant?.property_id || contract?.property_id || payment?.property_id || expense?.property_id || null;
...
export async function createAttachmentZipExport(ownerId: string) {
  const admin = getSupabaseAdmin(); const context = await loadContext(admin, ownerId); const rows = await loadRows(admin, ownerId);
  const files: Record<string, Uint8Array> = {}; const manifest: AttachmentExportManifestEntry[] = []; const usedPaths = new Set<string>(); const now = new Date();
  for (const { table, row } of rows) {
    const parent = parentFor(table, row); const provider = row.storage_provider === "supabase" || row.storage_provider === "google_drive" ? row.storage_provider : "unknown";
    const date = table === "property_files" ? row.uploaded_at : table === "contract_files" ? context.contracts.get(row.contract_id || "")?.start_date : table === "rent_payment_files" ? context.payments.get(row.rent_payment_id || "")?.payment_date || context.payments.get(row.rent_payment_id || "")?.rent_month : context.expenses.get(row.expense_id || "")?.payment_date || context.expenses.get(row.expense_id || "")?.expense_month;
  ...
  const zip = zipSync(files, { level: 6 }); const part = (value: number) => String(value).padStart(2, "0");
