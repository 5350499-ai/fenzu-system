...
export function isFreeSingleRestrictedModule(moduleKey: string) {
  return ["attachments", "partnership_settlement", "audit_logs", "accounts"].includes(moduleKey);
}
...
export function isFreeSingleRestrictedSensitivePermission(key: string) {
  return [
    ...
    "can_view_partnership_settlement", "can_view_audit_logs", "can_manage_accounts"
  ].includes(key);
}
