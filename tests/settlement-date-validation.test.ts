...
import { getMaxSettlementEndDate, getSettlementDateValidationError, getSettlementBusinessToday, isValidSettlementRange, validDate } from "../lib/settlement-date.ts";
...
test("settlement UI applies the same max-end boundary to custom and three-month ranges", () => {
  ...
  assert.match(page, /getMaxSettlementEndDate\(today\)/);
