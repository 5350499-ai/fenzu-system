...
test("real historical overlap remains the batch-level confirmation guard", () => {
  assert.match(page, /batch\.status === "confirmed" && selectedPropertyIds\.includes\(batch\.property_id\)/);
  assert.match(page, /if \(busy \|\| !settlement \|\| !selectedPropertyIds\.length \|\| overlap \|\| exactBatch/);
  assert.match(page, /!overlap && !exactBatch/);
});
