-- Canonical one-click check-in finance model.
-- Future rent payments store rent only; deposits remain separate records.
-- Existing rows and the 20-parameter occupant-count wrapper are unchanged.
begin;
...
begin
  ...
  insert into public.contracts (
    ...
    deposit_amount, start_date, end_date, coverage_start_date, coverage_end_date,
    status, notes
  ) values (
    ...
    p_contract_end_date, p_coverage_start_date, p_coverage_end_date,
    '有效', nullif(btrim(coalesce(p_notes, '')), '')
  );
  ...
  if coalesce(p_deposit_amount, 0) > 0 then
    ...
    v_deposit_notes := concat_ws(
      E'\n',
      case when v_payment_id is not null then '[收租押金:' || v_payment_id::text || ']' end,
      nullif(btrim(coalesce(p_notes, '')), '')
    );
  ...
  insert into public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'rentAmount', v_rent_due, 'depositAmount', coalesce(p_deposit_amount, 0),
...
alter function public.create_atomic_check_in(
