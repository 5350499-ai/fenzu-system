  ...
  const body = <>
    ...
    {variant === "full" && (display.debtCase?.canCollect || display.debtCase?.canWaive) ? (
      <span className="reminder-rent-actions">
        {display.debtCase?.canCollect ? <Link className="btn primary" href={`/rent-payments?collectPayment=${encodeURIComponent(display.debtCase.paymentId)}&overdue=1`}>登记补交</Link> : null}
