      ...
      <section className="card panel appointment-page-panel">
        ...
        {!loaded ? <p className="muted">正在加载预约...</p> : grouped.length ? <div className="appointment-sections">{groupedSections.map((section) => <section className="appointment-section" key={section.status}><div className="appointment-list">{section.items.map((item) => {
          ...
          if (location) return <article className="appointment-row" key={item.id}>
            ...
            <div className="appointment-row-actions"><QuickStatusMenu disabled={saving || !access.can("properties", "edit")} onChange={(status) => void updateStatus(item, status)} /></div>
          ...
          return <article className="appointment-row" key={item.id}>
            <div className="appointment-main-line"><strong>{formatManagementAppointmentDateTime(item.appointmentDate, item.appointmentTime)}</strong><span>{contactLabel(item)}</span><small className={`appointment-status status-${statusTone(item.status)}`}>{item.status}</small></div>
            <div className="appointment-meta-line"><span>{formatAppointmentLocation(property?.name, room?.roomNumber || room?.name)}</span>{item.notes ? <small className="appointment-note">{item.notes}</small> : null}</div>
            <div className="appointment-row-actions"><QuickStatusMenu disabled={saving || !access.can("properties", "edit")} onChange={(status) => void updateStatus(item, status)} /></div>
            <div className="appointment-actions"><button className="icon-button" type="button" aria-label="编辑预约" onClick={() => openForm(item)}><Edit3 size={16} /></button>{statusTone(item.status) === "converted" ? <button className="icon-button" type="button" aria-label="一键入住" onClick={() => { const params = new URLSearchParams({ fromViewing: "1", propertyId: item.propertyId || "", roomId: item.roomId || "", tenantName: item.contactName || "", phone: item.contactPhone || item.contactWhatsapp || "", notes: item.notes || "" }); window.location.href = `/check-in?${params.toString()}`; }}><LogIn size={16} /></button> : null}<button className="icon-button danger" type="button" aria-label="删除预约" onClick={() => void remove(item)} disabled={saving}><Trash2 size={16} /></button></div>
          </article>;
