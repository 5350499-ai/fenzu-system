...
export async function GET(request: Request) {
  try {
    ...
    const modulePermissions = emptyModulePermissions().map((base) => {
      ...
      if (freeSingle && base.moduleKey === "partnership_settlement") return { ...base, canView: true, canCreate: false, canEdit: false, canArchive: false, canDelete: false };
    ...
    return NextResponse.json({
      ...
      sensitivePermissions: context.profile.account_type === "owner"
        ...
        : Object.fromEntries(Object.entries(clientSensitivePermissions(sensitiveResult.data)).map(([key, value]) => [key, freeSingle && key === "canViewPartnershipSettlement" ? true : freeSingle && isFreeSingleWorkspaceOwner(context) && key === "canManageSettings" ? true : (freeSingle ? (isFreeSingleRestrictedSensitivePermission(key) ? false : value) : value)])),
