...
export async function POST(request: Request) {
  ...
  await admin.from("account_recovery_scheduler_runs").upsert({ schedule_slot: slot, status: "running", workspace_count: workspaces.length, started_at: now.toISOString() }, { onConflict: "schedule_slot" });
  ...
  await admin.from("account_recovery_scheduler_runs").update({ status, completed_at: new Date().toISOString(), success_count: successCount, failure_count: failureCount, error_summary: failures.join("; ") || null }).eq("schedule_slot", slot);
