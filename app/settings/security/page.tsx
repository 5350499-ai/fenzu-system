...
export default function AccountSecurityPage() {
  ...
  return <AppLayout title="账号安全" description="管理当前账号的邮箱和密码。"><section className="card panel password-security-panel"><div className="panel-header"><div><h2 className="panel-title">账号安全</h2><p className="muted">只允许修改当前登录账号，不显示或读取任何密码。</p></div><Link className="btn compact" href="/settings">返回设置</Link></div><div className="account-security-summary"><span>当前账号<strong>{access.profileDisplayName || access.profileUsername || "-"}</strong></span><span>邮箱<strong>{internalEmail ? "未绑定可接收邮件邮箱" : email || "未读取到邮箱"}</strong></span><span>邮箱状态<strong>{internalEmail ? "请联系管理员" : verified ? "已验证" : "未验证"}</strong></span></div><form className="form-grid" onSubmit={submit}><div className="field"><label htmlFor="security-current-password">当前密码</label><PasswordInput id="security-current-password" name="current-password" autoComplete="current-password" value={currentPassword} onValueChange={setCurrentPassword} /></div><div className="field"><label htmlFor="security-new-password">新密码</label><PasswordInput id="security-new-password" name="new-password" autoComplete="new-password" value={newPassword} onValueChange={setNewPassword} /></div><div className="field"><label htmlFor="security-password-confirmation">确认新密码</label><PasswordInput id="security-password-confirmation" name="new-password-confirmation" autoComplete="new-password" value={confirmation} onValueChange={setConfirmation} /></div>{notice ? <p className={notice.includes("成功") ? "success-text" : "danger-text"}>{notice}</p> : null}<div className="settings-actions"><button className="btn primary" type="submit" disabled={saving} aria-busy={saving}>{saving ? "保存中…" : "保存修改"}</button><button className="btn" type="button" onClick={() => { setCurrentPassword(""); setNewPassword(""); setConfirmation(""); }}>清空</button></div></form></section></AppLayout>;
}
