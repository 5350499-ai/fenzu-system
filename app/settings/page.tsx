...
function buildCsvExport(data: BackupData, settlement: ReturnType<typeof buildSettlementRows>) {
  return [
    ...
    csvSection("租客", ["id", "房源ID", "房间ID", "姓名", "电话", "微信", "来源", "月租", "押金标准", "每月缴费日", "状态", "备注"], data.tenants.map((item) => [item.id, item.propertyId, item.roomId, item.name, item.phone, item.wechat, item.source, item.monthlyRent, item.depositAmount, item.paymentDay ?? "", item.status, item.notes || ""])),
