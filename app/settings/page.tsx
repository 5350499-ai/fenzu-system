...
function buildCsvExport(data: BackupData, settlement: ReturnType<typeof buildSettlementRows>) {
  return [
    ...
    csvSection("收租", ["id", "月份", "租客ID", "月租参考", "实收金额", "差额参考", "覆盖开始", "覆盖结束", "付款方式", "收款归属", "欠费", "备注"], data.rentPayments.map((item) => [item.id, item.rentMonth, item.tenantId, item.amountDue, item.amountPaid, item.amountUnpaid, item.coverageStartDate || "", item.coverageEndDate || "", item.paymentMethod, item.receivedBy || "A", item.isOverdue ? "是" : "否", item.notes || ""])),
