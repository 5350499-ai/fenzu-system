...
export default function DepositsPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const next = form.id
      ? deposits.map((deposit) => (deposit.id === form.id ? form : deposit))
      : [{ ...form, id: crypto.randomUUID() }, ...deposits];
    await persist(next);
    close();
  }
    ...
    <AppLayout title="押金管理" description="记录押金收取、退还、扣除和状态。真实押金记录建议作废，不建议删除。">
      <section className="card panel">
        ...
        <div className="table-wrap">
          <table>
            <thead><tr><th>日期</th><th>房间</th><th>租客</th><th>金额</th><th>状态</th><th>类型</th><th>备注</th><th>操作</th></tr></thead>
