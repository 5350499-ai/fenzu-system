        ...
        {activeAccount?.accountType === "owner" ? <p className="muted">主管理员资料受数据库保护，不可在此修改。</p> : (
          <>
            <details className="account-section" open><summary>基本资料</summary><div className="form-grid">
              ...
              {!editor.id ? <><div className="field"><label>初始密码</label><PasswordInput autoComplete="new-password" value={editor.password} onValueChange={(password) => setEditor({ ...editor, password })} /></div><div className="field"><label>确认密码</label><PasswordInput autoComplete="new-password" value={editor.passwordConfirmation} onValueChange={(passwordConfirmation) => setEditor({ ...editor, passwordConfirmation })} /></div></> : null}
