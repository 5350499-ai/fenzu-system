...
export default function GoogleAttachmentMigrationPage() {
  ...
  return <AppLayout title="Google Drive 历史附件迁移" description="管理员只读扫描与受控迁移。">
    ...
    {confirmOpen ? <ModalPortal><div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="migration-confirm-title"><section className="card panel modal-card"><h2 id="migration-confirm-title" className="panel-title">确认迁移</h2><p className="muted">将复制 {summary?.migratable ?? 0} 个附件并切换索引。Google 原文件不会删除。</p>{runError ? <p className="error-text">{runError}</p> : null}{runResult ? <p className="success-text">已迁移 {runResult.summary.migrated} 条；跳过 {runResult.summary.skipped} 条；失败 {runResult.summary.failed} 条。</p> : null}<div className="button-row"><button className="btn" type="button" onClick={() => setConfirmOpen(false)} disabled={running}>关闭</button><button className="btn primary" type="button" onClick={() => void runMigration()} disabled={running}>{running ? "迁移中…" : "确认迁移"}</button></div></section></div></ModalPortal> : null}
  </AppLayout>;
}
