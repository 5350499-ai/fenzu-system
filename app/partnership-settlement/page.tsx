...
export default function PartnershipSettlementPage() {
  ...
  return <AppLayout title="合伙结算" description="按房源、比例生效区间和真实收支归属生成动态合伙结算。">
    ...
    {!result ? <section className="card panel"><p className="muted">请选择条件并点击“开始试算”后查看结算结果。</p></section> : <>
      <section className="card compact-report-card"><CompactMetric label="总收入" value={euro(result.totalIncome)} /><CompactMetric label="总支出" value={euro(result.totalExpense)} /><CompactMetric label="净利润" value={euro(result.netProfit)} tone={result.netProfit < 0 ? "danger" : "profit"} /><CompactMetric label="结算次数" value={batches.length ? `${batches.length}次` : "0次（首次结算前）"} href="/partner-settlements" /></section>
...
function CompactMetric({ label, value, tone, href }: { label: string; value: string; tone?: "danger" | "profit"; href?: string }) { const content = <><span>{label}</span><strong className={tone === "danger" ? "danger-text" : tone === "profit" ? "profit" : ""}>{value}</strong></>; return href ? <Link className="compact-report-metric compact-report-metric--link" href={href}>{content}</Link> : <div className="compact-report-metric">{content}</div>; }
