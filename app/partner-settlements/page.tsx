...
function SettlementHistoryCard({ batch, propertyName }: { batch: Batch; propertyName: string }) {
  ...
  async function toggle(event: SyntheticEvent<HTMLDetailsElement>) {
    ...
    setLoading(true);
    try { const session = await getValidSupabaseSession(); if (!session) throw new Error("登录已失效，请重新登录"); const response = await fetch(`/api/partner-settlements/${batch.id}`, { headers: { Authorization: `Bearer ${session.access_token}` }, cache: "no-store" }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error || "快照加载失败"); setSnapshot(payload); }
    catch (error) { setSnapshot({ error: error instanceof Error ? error.message : "快照加载失败" }); }
...
function InlineSnapshot({ snapshot }: { snapshot: any }) {
  const batch = snapshot.batch; const income = Array.isArray(batch.income_details_snapshot) ? batch.income_details_snapshot : []; const expense = Array.isArray(batch.expense_details_snapshot) ? batch.expense_details_snapshot : [];
  return <div className="snapshot-inline"><p className="muted">确认时间：{new Date(batch.confirmed_at).toLocaleString("zh-CN")} · 确认账号：{batch.confirmed_by_display_name_snapshot || batch.confirmed_by_account_id || "未保存"}</p><p>汇总：收入 {euro(Number(batch.total_income))} · 支出 {euro(Number(batch.total_expense))} · 净利润 {euro(Number(batch.net_profit))}</p><h4>比例分段</h4>{snapshot.segments.map((segment: any) => <p key={segment.id}>{segment.segment_start} 至 {segment.segment_end} · 收入 {euro(Number(segment.total_income))} · 支出 {euro(Number(segment.total_expense))} · 净利润 {euro(Number(segment.net_profit))}<br /><small>{JSON.stringify(segment.shares_snapshot)}</small></p>)}<h4>合伙人结算明细</h4>{snapshot.partners.map((partner: any) => <p key={partner.id}>{partner.partner_display_name_snapshot}：代收 {euro(Number(partner.actual_collected))} · 垫付 {euro(Number(partner.actual_paid))} · 留存 {euro(Number(partner.actual_retained))} · 应得 {euro(Number(partner.profit_entitlement))} · 余额 {euro(Number(partner.settlement_balance))}</p>)}<h4>转账建议</h4>{snapshot.transfers.length ? snapshot.transfers.map((transfer: any) => <p key={transfer.id}>{transfer.from_name_snapshot} → {transfer.to_name_snapshot}：{euro(Number(transfer.amount))}</p>) : <p className="muted">无需转账</p>}<h4>逐笔明细</h4>{income.length || expense.length ? <><p>收入：{income.length}笔 · 支出：{expense.length}笔</p>{income.map((item: any, index: number) => <p key={item.paymentId || index}>{item.date} · {item.partnerName} · {euro(Number(item.amount))}</p>)}{expense.map((item: any, index: number) => <p key={item.expenseId || index}>{item.date} · {item.partnerName} · {euro(Number(item.amount))}</p>)}</> : <p className="muted">该快照确认时未保存逐笔收入/支出明细，不能用当前账目补造。</p>}</div>;
}
