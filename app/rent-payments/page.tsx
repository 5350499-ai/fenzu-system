...
export default function RentPaymentsPage() {
  ...
  useEffect(() => {
    ...
    const propertyScope = parseAnalyticsPropertyScope(window.location.search);
    if (propertyScope !== null) {
      setPropertyScopeRequested(true);
      setSelectedPropertyIds(propertyScope);
    }
        ...
        <ModalPortal><div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
          <section className="card modal-card" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <form className="form-grid rent-payment-form-grid" onSubmit={submit}>
              ...
              {isRentPayment(form) ? <div className="form-grid-row rent-payment-coverage-row" data-layout-row="coverage">
                <div className="field"><label>租金覆盖开始日期</label><input required type="date" value={form.coverageStartDate || ""} onChange={(event) => { const coverageStartDate = event.target.value; setForm((current) => ({ ...current, coverageStartDate, coverageEndDate: !current.coverageEndDate || current.coverageEndDate < coverageStartDate ? defaultCoverageEnd(coverageStartDate) : current.coverageEndDate })); }} /></div>
                <div className="field"><label>租金覆盖结束日期</label><input required type="date" min={form.coverageStartDate || undefined} value={form.coverageEndDate || ""} onChange={(event) => setForm((current) => ({ ...current, coverageEndDate: event.target.value }))} /></div>
              </div> : null}
