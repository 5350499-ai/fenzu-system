...
export default function DataCenterPage() {
  ...
  return <AppLayout title="备份与恢复" description="管理业务数据备份、恢复入口和报表导出。">
    ...
    {exportSheetOpen ? <ModalPortal><div className="data-center-sheet-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) setExportSheetOpen(false); }}><section className="data-center-sheet" role="dialog" aria-modal="true" aria-labelledby="export-sheet-title" onClick={(event) => event.stopPropagation()}><div className="data-center-sheet-handle" aria-hidden="true" /><h2 id="export-sheet-title">请选择导出格式</h2><button type="button" className="data-center-sheet-option" onClick={() => exportTable("excel")}><FileSpreadsheet size={19} /><span>Excel <small>推荐</small></span></button><button type="button" className="data-center-sheet-option" onClick={() => exportTable("csv")}><FileText size={19} /><span>CSV <small>兼容其它软件</small></span></button><SecondaryButton type="button" onClick={() => setExportSheetOpen(false)}>取消</SecondaryButton></section></div></ModalPortal> : null}
    {subscriptionDialog ? <ModalPortal><div className="data-center-dialog-backdrop" role="presentation" onClick={(event) => { if (event.target === event.currentTarget) setSubscriptionDialog(null); }}><section className="data-center-dialog" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><div className="data-center-dialog-icon"><Crown size={22} /></div><h2>订阅后即可使用</h2><ul><li>自动云备份</li><li>历史恢复</li><li>更多云端能力</li></ul><p className="data-center-muted">{subscriptionDialog === "restore" ? "恢复前系统将自动创建一份当前数据备份。" : "自动云备份功能将在后续阶段开放。"}</p><SecondaryButton type="button" onClick={() => setSubscriptionDialog(null)}>知道了</SecondaryButton></section></div></ModalPortal> : null}
  </AppLayout>;
}
